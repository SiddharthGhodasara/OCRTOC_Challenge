...
weightsPath = os.path.sep.join([package_path, 'yolo', "yolov3-tiny-obj_best.weights"]) #/home/gaurav/ocrtoc_ws/src/3D_Pose_Estimation/yolo/yolov3-tiny-obj_best.weights
...
class do_task:
    ...
    def get_grasps_coord(self, label):
        ...
        while not rospy.is_shutdown():
            ...
            if (grasps_plots != "" and grasps_plots_list[0] > 2):
                ...
                if int(grasps_plots_list[0]) < 90: #without scaling search area, the threshold was 70
	...
	req_distance = tot_distance - 0.06 #0.02 # For nearby objects, offset is lesser
	z=0.18
	pitch = 1.37#1.06
	...
	#	r,p,y= (roll + 90, 90, 0)
        ...
        grasp_pose.pose.position.y = position_y #+ 0.03
        ...
        waypoints = []
        wpose = self.group.get_current_pose().pose
        wpose.position.z += position_z + 0.07
        waypoints.append(copy.deepcopy(wpose))
        (cartesian_plan, fraction) = self.group.compute_cartesian_path(waypoints,0.01, 0.0)
        self.group.execute(cartesian_plan, wait=True)
