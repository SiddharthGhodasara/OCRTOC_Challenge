...
def prediction(image):
    ...
    if len(idxs) > 0:
    	...
    	for i in idxs.flatten():
            ...
            #Converting to format in which we need to publish
            label_str += str(label) + str(',') + str(world_x) +  str(',') + str(world_y) + str(',') + str(world_z) + str(';')
            ...
            rmat=np.array([])
            rmat_1, rmat_2 = cv2.Rodrigues(rotation_vector, rmat)
            #print(rmat.shape())
            #print(label, translation_vector)
            #print(label, rmat_1)
            eu_angles = tf.transformations.euler_from_matrix(rmat_1, axes='sxyz')
            quat_vector = quaternion_from_euler(eu_angles[0],eu_angles[1], eu_angles[2])
                #Making Point Stamp Message
            obj_pose = geometry_msgs.msg.PoseStamped()
            obj_pose.header.frame_id = camera_frame
            #point = geometry_msgs.msg.Pose()
            obj_pose.pose.position.x =  translation_vector[0]
            obj_pose.pose.position.y =  translation_vector[1]
            obj_pose.pose.position.z =  translation_vector[2]

            obj_pose.pose.orientation.x =  quat_vector[0]
            obj_pose.pose.orientation.y =  quat_vector[1]
            obj_pose.pose.orientation.z =  quat_vector[2]
            obj_pose.pose.orientation.w =  quat_vector[3]
            ...
            #Applying the transform
            obj_transformed = tf2_geometry_msgs.do_transform_pose(obj_pose, transform)
            #Returning the transform coordinates
            print(label, obj_transformed)
