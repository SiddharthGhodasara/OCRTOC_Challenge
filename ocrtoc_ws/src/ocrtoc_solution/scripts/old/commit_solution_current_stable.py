...
"rubiks_cube":[0.059545,0.058733,0.057940], "mustard_bottle":[0.067913,0.067755,0.101856]}
...
def get_pose (img):
	...
	for i in range(len(contours)):
		...
		if area > 10:
			...
			box = np.int0(box)
			length = ((box[1][0] - box[0][0])**2 + (box[1][1] - box[0][1])**2)**0.5
			width = ((box[2][0] - box[1][0])**2 + (box[2][1] - box[1][1])**2)**0.5
		
			if length > width:
				angle = math.atan2((box[1][1] - box[0][1]), (box[1][0] - box[0][0]))
			 	
			else:
				angle = math.atan2((box[2][1] - box[1][1]), (box[2][0] - box[1][0])) 
...
#Failure case: The robot doesn't grasp object - go back to task pose to save time 
	...
	plan3 = group_1.go(wait=True)
	'''
	#Getting pose from realsense
	rs_image = rospy.wait_for_message('/realsense/color/image_raw', Image)
	try:
		#Converting Sensor Image to cv2 Image
		rs_image_converted = bridge.imgmsg_to_cv2(rs_image, 'bgr8')
		//yaw = math.radians(get_pose(rs_image_converted))
		if (abs(r-yaw) < 0.25 or abs(r+math.pi/2 - yaw) < 0.25): 
