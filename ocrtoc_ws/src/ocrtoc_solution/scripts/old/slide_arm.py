...
y_gripper = group.get_current_pose().pose.position.y
group.set_named_target("pose1")
plan3 = group.go(wait=True)

diff = y_gripper - obj_y
print(diff)
...
if round(obj_y,4) < 0:
	...
	wpose.position.y -= scale * diff 
	waypoints.append(copy.deepcopy(wpose))
	(cartesian_plan, fraction) = group.compute_cartesian_path(waypoints,0.01, 0.0)
	group.execute(cartesian_plan, wait=True)

else: 
	print("take slide pose 2")
	pose_target = geometry_msgs.msg.Pose()
	group.set_named_target("slide_pose2_assist")
	plan3 = group.go(wait=True)
	group.set_named_target("slide_pose2_main")
	plan3 = group.go(wait=True)
