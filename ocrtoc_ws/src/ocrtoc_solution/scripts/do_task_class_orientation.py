...
def check_curr_pose(pose1):
    ...
    root = ET.parse('/home/kaushik/ocrtoc_materials/scenes/1-1/input.world').getroot()
    ...
    for child in root.findall('world/model'):
        ...
        for pose in child.findall('pose'):
            ...
            #Linear Coordinates
            t_x = pose1.pose.position.x - var_list[0]
            t_y = pose1.pose.position.y - var_list[1]
            t_z = pose1.pose.position.z - var_list[2]
            #Converting euler angles to quaternion
            quaternion = tf.transformations.quaternion_from_euler(var_list[3], var_list[4], var_list[5])
            #Rotational Coordinates
            o_x = pose1.pose.orientation.x - quaternion[0]
            o_y = pose1.pose.orientation.y - quaternion[1]
            o_z = pose1.pose.orientation.z - quaternion[2]
            o_w = pose1.pose.orientation.w - quaternion[3]

            return (t_x, t_y, t_z), (o_x, o_y, o_z, o_w)
...
class do_task:
    ...
    def feedback(self, goal, i): 
        ...
        if abs(pose_curr_x - goal.pose_list[i].position.x) <= 0.05 and abs(pose_curr_y - goal.pose_list[i].position.y) <= 0.05:
            ...
            if abs(pose.pose.orientation.x -  goal.pose_list[i].orientation.x) <= 0.15 and abs(pose.pose.orientation.y -  goal.pose_list[i].orientation.y) <= 0.15 and abs(pose.pose.orientation.z -  goal.pose_list[i].orientation.z) <= 0.15 and abs(pose.pose.orientation.w -  goal.pose_list[i].orientation.w) <= 0.15:
                ...
                print("Error in z", abs(pose.pose.orientation.z -  goal.pose_list[i].orientation.z))
                print("Error in w", abs(pose.pose.orientation.w -  goal.pose_list[i].orientation.w))
                print("Bad orientation, changing: ")
                goal, i = self.task(goal, i, 1)
            return 1
    ...
    def task(self, goal, i, orientation):
        ...
        print("Transalation error with initial pose - {}".format(c_e_t))
        print("Orientation error with initial pose - {}".format(c_o_t))
