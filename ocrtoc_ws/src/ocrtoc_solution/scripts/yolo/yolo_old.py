...
weightsPath = os.path.sep.join(['yolo-coco', "yolov3-tiny-obj_5000.weights"])
configPath = os.path.sep.join(['yolo-coco', "yolov3-tiny-obj.cfg"])
...
def prediction(image):
    ...
    print("Got label {}".format(l.data))
    #sus = rospy.Subscriber('/label', String, cb)
    #print(l)
    ...
    if len(idxs) > 0:
    	...
    	for i in idxs.flatten():
            ...
            if label == l.data:
                ...
                if (label == 'master_chef_can' or label == 'potted_meat_can' or label == 'pudding_box'): 
                    cx += 5
                    cy += 5
