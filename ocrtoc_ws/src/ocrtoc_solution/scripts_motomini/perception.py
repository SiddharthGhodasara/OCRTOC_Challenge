...
class Perception:
	...
	def transform_frames(self):
		...
		trans1, quat1 = tf.TransformListener().lookupTransform("world", self.camera_frame, rospy.Time())
		...
		if len(idxs) > 0:
			...
			for i in idxs.flatten():
				...
				if req_label == label:
					...
					#print("For ", label)
					#print("YOLO Pose", self.pose_wrt_world)
					color = (0,0,255)
					cv2.rectangle(self.image, (x , y), (x + w, y + h ), color, 2)
					text = "{}".format(label)
					cv2.putText(self.image, text, (x, y - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
					...
					if req_type == '3D':
						...
						self.box_img = self.image[y-10 : y+h+10, x-10 : x+w+10]
						
						#Converting to RGB Color Space
						self.box_img = cv2.cvtColor(self.box_img, cv2.COLOR_BGR2RGB)
						...
						source, target, source_down, target_down, source_fpfh, target_fpfh = self.prepare_dataset(self.mesh_pointcloud, self.rgbd_pointcloud, self.transform_frames())

						#Global Registration
						result_ransac = self.execute_global_registration(source_down, target_down,source_fpfh, target_fpfh)
						
						o3d.visualization.draw_geometries([self.mesh_pointcloud.transform(result_ransac), self.rgbd_pointcloud])
						#Local Refinement
