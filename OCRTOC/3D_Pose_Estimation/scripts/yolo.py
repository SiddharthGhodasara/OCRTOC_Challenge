...
image_topic = "/camera/rgb/image_raw"#"/kinect2/sd/image_color_rect" #rospy.get_param("/image_topic")
camera_topic =  "/camera/rgb/camera_info"#"/kinect2/sd/camera_info" #rospy.get_param('/camera_topic')
depth_topic = "/camera/depth_registered/image"#"/camera/depth_registered/image_raw"#"/kinect2/sd/image_depth_rect" #rospy.get_param('/depth_topic')
camera_frame = "camera_rgb_optical_frame"#"kinect2_rgb_optical_frame" #rospy.get_param('/camera_frame')
...
for file in os.listdir(pcd_path):
    ...
    s = o3d.io.read_point_cloud(path)
    file_label = str(file).split('.')[0]
    meshes_dir[file_label] = s
print(meshes_dir.keys())
...
def draw_registration_result(source, target, transformation):
    ...
    #target_temp.paint_uniform_color([0, 0.651, 0.929]) rgb
...
def uv_to_xyz(cx, cy):
    ...
    #print(depth.data)
    print("Obtained depth images")
    #depth_img  = bridge.imgmsg_to_cv2(depth, depth.encoding)
    #depth_img = np.asarray(depth_img, dtype="Float32")
    ...
    print ("Co-ord wrt world are:", pose_transformed.point.x, pose_transformed.point.y, pose_transformed.point.z )
...
#Prediction Function
def prediction(image, req_label):
    #Getting the image dimensions
    (H, W) = image.shape[:2]

    #req_label = rospy.wait_for_message('/label', String)

    global focal_length
    global center
    global camera_matrix

    #center = (255.7303009033203, 206.33770751953125)#(W/2, H/2)
    center = (319.5, 239.5)
    #focal_length = 366.2605895996094#center[0] / np.tan(60/2 * np.pi / 180)
    focal_length = 525.0
    ...
    if len(idxs) > 0:
    	...
    	for i in idxs.flatten():
            ...
            if label == req_label:
                ...
                try:
                    depth_converted = bridge.imgmsg_to_cv2(depth, desired_encoding='passthrough')
                    #depth_converted  = bridge.imgmsg_to_cv2(depth, depth.encoding)
                    #depth_converted = np.asarray(depth_converted, dtype="Float32")
                     
                except CvBridgeError, e:
                    print e

                #Converting to np array
                depth_array = np.array(depth_converted)
                #depth_converted = np.true_divide(depth_array,1000)

                #Extracting the depth image of the bounding box
                depth_box = depth_converted[y-10 : y+h+10, x-10 : x+w+10]
                ...
                (H, W) = image.shape[:2]
                #center = (319.5, 239.5)
                center_x = 319.5#255.7303009033203#319.5 #640.5
                center_y = 239.5#206.33770751953125#239.5 #360.5
                focal_length_y = 525.0#366.2605895996094#525 #1120.1199067175087
                focal_length_x = 525.0#366.2605895996094#525 #1120.1199067175087
                ...
                o3d.visualization.draw_geometries([pcd,s])
                ...
                # xy_pose.header.frame_id = str(label) + "," + str(0.01) + "," + str(0.01) + "," + str(0.01)
...
def main():
    ...
    label_sub = rospy.Subscriber('/label_1', String, label_cb)
